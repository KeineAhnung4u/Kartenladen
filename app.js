...
function addItem(form) {
  ...
  state.items.push({
    ...
    set: form.set.value, type: form.type.value, rarity: form.rarity.value, subtype: form.subtype.value
  });
  save();
  form.reset();
...
function addPurchase(form) {
  ...
  showToast('Ankauf erfasst' + (shouldAddToStock ? ' (+ Lager)' : ''));
  render();
}

function sellItemFromLager(itemId) {
  const item = state.items.find(i => i.id === itemId);
  if (!item || item.qty <= 0) return;
  addToCart(itemId);
  view = 'kasse';
  ...
  showToast('In den Verkauf gelegt');
}
...
async function loadApiFilterOptions() {
  ...
  for (const ep of endpoints) {
    try {
      ...
      apiFilterOptions[ep.key] = items.map(i => typeof i === 'string' ? i : i.name).filter(Boolean).sort();
    } catch (e) { console.warn('API-Filter fehlgeschlagen', ep.key, e); }
  }
  render();
}
...
function buildPriceQuery(term) {
  ...
  const looksLikeApiQuery = /^(name:|set\.name:|types:|rarity:|subtypes:)|[():]/.test(cleaned) || /\b(and|or|not)\b/i.test(cleaned);
...
async function searchPrice(query) {
  ...
  try {
    ...
    priceResults = 'error';
    priceErrorMessage = 'Preisabfrage fehlgeschlagen.';
  }
  ...
  <div class="row" style="align-items:stretch;">
    <div class="card" style="flex:1.3;min-width:280px;">
      ...
      <div class="row" style="margin-bottom:10px;">
        <div class="field"><input class="search-input" data-search-kasse value="${esc(searchState.kasse)}" placeholder="Suche..."></div>
      ...
      <div class="row" style="margin-bottom:10px;">
        <div class="field"><select class="search-input" data-kasse-filter="set"><option value="">Alle Sets</option>${apiFilterOptions.sets.map(opt => `<option value="${esc(opt)}" ${searchState.kasseSet === opt ? 'selected' : ''}>${esc(opt)}</option>`).join('')}</select></div>
        <div class="field"><select class="search-input" data-kasse-filter="type"><option value="">Alle Typen</option>${apiFilterOptions.types.map(opt => `<option value="${esc(opt)}" ${searchState.kasseType === opt ? 'selected' : ''}>${esc(opt)}</option>`).join('')}</select></div>
        <div class="field"><select class="search-input" data-kasse-filter="rarity"><option value="">Alle Rarities</option>${apiFilterOptions.rarities.map(opt => `<option value="${esc(opt)}" ${searchState.kasseRarity === opt ? 'selected' : ''}>${esc(opt)}</option>`).join('')}</select></div>
        <div class="field"><select class="search-input" data-kasse-filter="subtype"><option value="">Alle Subtypes</option>${apiFilterOptions.subtypes.map(opt => `<option value="${esc(opt)}" ${searchState.kasseSubtype === opt ? 'selected' : ''}>${esc(opt)}</option>`).join('')}</select></div>
      </div>
    ...
    <div class="card" style="flex:1;min-width:260px;">
      <h2>Warenkorb</h2>
      ${cart.length === 0 ? '<div class="empty">Leer</div>' :
      cart.map(c => {
        const item = state.items.find(i => i.id === c.itemId);
      ...
      <button class="btn-gold" style="width:100%;margin-top:12px;" id="checkoutBtn" ${cart.length === 0 ? 'disabled' : ''}>Abschließen</button>
    </div>
  </div>`;
}
...
function renderLager() {
  ...
  <div class="card">
    ...
    <form id="addItemForm">
      <div class="row">
        ...
        <div class="field" style="max-width:120px;"><label>EK</label><input name="buyPrice" type="number" step="0.01" min="0" value="0"></div>
        <div class="field" style="max-width:120px;"><label>VK</label><input name="sellPrice" type="number" step="0.01" min="0" value="0"></div>
  ...
  <div class="card">
    <h2>Bestand (${filtered.length}/${state.items.length})</h2>
    <div class="row" style="margin-bottom:10px;">
      <div class="field"><input class="search-input" data-search-lager value="${esc(searchState.lager)}" placeholder="Suche..."></div>
      <div class="field" style="max-width:140px;">
    ...
    <div class="row" style="margin-bottom:10px;">
      <div class="field"><select class="search-input" data-lager-filter="set"><option value="">Alle Sets</option>${apiFilterOptions.sets.map(opt => `<option value="${esc(opt)}" ${searchState.lagerSet === opt ? 'selected' : ''}>${esc(opt)}</option>`).join('')}</select></div>
      <div class="field"><select class="search-input" data-lager-filter="type"><option value="">Alle Typen</option>${apiFilterOptions.types.map(opt => `<option value="${esc(opt)}" ${searchState.lagerType === opt ? 'selected' : ''}>${esc(opt)}</option>`).join('')}</select></div>
      <div class="field"><select class="search-input" data-lager-filter="rarity"><option value="">Alle Rarities</option>${apiFilterOptions.rarities.map(opt => `<option value="${esc(opt)}" ${searchState.lagerRarity === opt ? 'selected' : ''}>${esc(opt)}</option>`).join('')}</select></div>
      <div class="field"><select class="search-input" data-lager-filter="subtype"><option value="">Alle Subtypes</option>${apiFilterOptions.subtypes.map(opt => `<option value="${esc(opt)}" ${searchState.lagerSubtype === opt ? 'selected' : ''}>${esc(opt)}</option>`).join('')}</select></div>
    </div>
    ${filtered.length === 0 ? '<div class="empty">Keine Artikel</div>' :
...
function renderAnkauf() {
  ...
  <div class="card">
    ...
    <form id="addPurchaseForm">
      <div class="row">
        ...
        <div class="field" style="max-width:130px;"><label>Preis</label><input name="pprice" type="number" step="0.01" min="0" value="${esc(prefilledPrice)}"></div>
        <div class="field" style="max-width:160px;display:flex;align-items:center;gap:6px;padding-bottom:8px;">
          ...
          <label for="addToStock" style="margin:0;text-transform:none;font-size:12.5px;">Ins Lager</label>
        </div>
  ...
  <div class="card">
    ...
    <div class="row" style="margin-bottom:10px;">
      <div class="field"><input class="search-input" data-search-ankauf value="${esc(searchState.ankauf)}" placeholder="Suche..."></div>
    </div>
    <div class="row" style="margin-bottom:10px;">
      <div class="field"><select class="search-input" data-ankauf-filter="set"><option value="">Alle Sets</option>${apiFilterOptions.sets.map(opt => `<option value="${esc(opt)}" ${searchState.ankaufSet === opt ? 'selected' : ''}>${esc(opt)}</option>`).join('')}</select></div>
      <div class="field"><select class="search-input" data-ankauf-filter="type"><option value="">Alle Typen</option>${apiFilterOptions.types.map(opt => `<option value="${esc(opt)}" ${searchState.ankaufType === opt ? 'selected' : ''}>${esc(opt)}</option>`).join('')}</select></div>
      <div class="field"><select class="search-input" data-ankauf-filter="rarity"><option value="">Alle Rarities</option>${apiFilterOptions.rarities.map(opt => `<option value="${esc(opt)}" ${searchState.ankaufRarity === opt ? 'selected' : ''}>${esc(opt)}</option>`).join('')}</select></div>
      <div class="field"><select class="search-input" data-ankauf-filter="subtype"><option value="">Alle Subtypes</option>${apiFilterOptions.subtypes.map(opt => `<option value="${esc(opt)}" ${searchState.ankaufSubtype === opt ? 'selected' : ''}>${esc(opt)}</option>`).join('')}</select></div>
    </div>
    ${filteredPurchases.length === 0 ? '<div class="empty">Keine Ankäufe</div>' :
...
function renderStatistik() {
  ...
  <div class="stat-grid">
    ...
    <div class="stat"><div class="lbl">Verkauft</div><div class="val">${t.unitsSold}</div></div>
  ...
  <div class="card">
    ...
    `<div class="bars">${days.map(d => `
      <div class="bar-wrap">
        <div class="bar" style="height:${Math.max(4, (d.total / maxVal) * 100)}px;" title="${eur(d.total)}"></div>
...
function addCardToStock(card) {
  ...
  save();
  render();
  showToast('Ins Lager übernommen');
}
  ...
  <div class="card">
    ...
    <form id="githubForm">
      <div class="row">
        ...
        <div class="field"><label>Repository</label><input name="repo" value="${esc(githubConfig.repo)}"></div>
      </div>
      <div class="row" style="margin-top:8px;">
        <div class="field" style="flex:2;"><label>Token</label><input name="token" type="password" value="${esc(githubConfig.token)}"></div>
        <div class="field"><label>Branch</label><input name="branch" value="${esc(githubConfig.branch)}"></div>
      </div>
      <div class="row" style="margin-top:12px;">
        <button type="submit" class="btn-gold">Speichern & Testen</button>
        <button type="button" id="syncNowBtn" ${!isGithubConfigured() ? 'disabled' : ''}>Jetzt syncen</button>
        <button type="button" id="clearGithubBtn" class="btn-red">Trennen</button>
      </div>
    </form>
    ...
    ${githubConfig.lastError ? `<div style="margin-top:8px;font-size:12px;color:#c23b3b;">Fehler: ${esc(githubConfig.lastError)}</div>` : ''}
  </div>`;
}
...
async function exportBackup() {
  ...
  const filename = `pokeshop-backup-${todayStr()}.txt`;
...
function attachHandlers() {
  ...
  if (clearGithubBtn) clearGithubBtn.addEventListener('click', () => {
    if (!confirm('Wirklich trennen?')) return;
